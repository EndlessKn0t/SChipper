...
// import Schipper.Instruction._
// import Schipper.InstructionType._

// class DecoderPort(bitWidth: Int = 32) extends Bundle {
//   val op       = Input(UInt(bitWidth.W))  // 32-bit instruction to decode
//   val inst     = Output(Instruction())    // Decoded instruction type (e.g., ADD, BEQ, etc.)
//   val rd       = Output(UInt(5.W))        // Destination register address
//   val rs1      = Output(UInt(5.W))        // Source register 1 address
//   val rs2      = Output(UInt(5.W))        // Source register 2 address
//   val imm      = Output(SInt(bitWidth.W)) // Immediate value (sign-extended if applicable)
//   val toALU    = Output(Bool())           // Flag to indicate if the instruction targets the ALU
//   val branch   = Output(Bool())           // Flag to indicate if the instruction involves a branch
//   val use_imm  = Output(Bool())           // Flag to indicate if the instruction uses an immediate
//   val jump     = Output(Bool())           // Flag to indicate if the instruction involves a jump
//   val is_load  = Output(Bool())           // Flag to indicate if the instruction loads data from memory
//   val is_store = Output(Bool())           // Flag to indicate if the instruction stores data to memory
// }

// /**
//  * Decoder module decodes RISC-V instructions into their constituent fields and signals.
//  * It supports various instruction types, including R, I, S, B, U, and J formats.
//  *
//  * @param bitWidth The bit width of the instruction (default is 32 for RV32I).
//  */
